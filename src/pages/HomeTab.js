...
const HomeTab = (props) => {
  ...
  // returns the 10 best Haks in terms of total points
  function bestHaks(){
  ...
  // returns the Hak with the biggest positive change in points from the day before
  function upHaks(){
    let tmp = Object.create(data)
  ...
  // returns the Hak with the biggest negative change in points from the day before
  function downHaks(){
    let tmp = Object.create(data)
    return tmp.sort((a, b) => (b.points[b.points.length-1] - b.points[b.points.length-2]) - 
  ...
  // put the best Haks in a side scrolling box
